...
const Sidenav = ({ children }: { children: React.ReactNode })  => {
  ...
  const collapsedVisible = ['/dashboard', '/files', '/recent', '/starred', '/shared']

  const desktopview = () => {
    const itemsToRender: SideItem[] = isCollapsed
      ? sidelinks.filter((s) => !('links' in s) && collapsedVisible.includes((s as LinkItem).href))
      : sidelinks
    ...
    return (
      <nav className="flex flex-col space-y-1">
        {itemsToRender.map((section) => {
          ...
          const isCenteredIcon = isCollapsed && collapsedVisible.includes(single.href)
          ...
          return (
            <div key={key}>
              <Link href={single.href} className={linkClass}>
                ...
                {!isCollapsed && (<span className="text-sm">{single.label}</span>)}
              </Link>
            </div>
          )
        })}
  ...
  return (
    <div className="flex min-h-screen bg-gradient-to-br from-[#02427E] to-[#05081A]">
      {/* Desktop Sidebar */}
      <div className={`hidden lg:flex flex-col border-r border-border/80 bg-gradient-to-br from-[#02427E] to-[#05081A] p-4 text-white transition-all duration-300 ${mode === 'open' ? 'w-56' : mode === 'collapsed' ? 'w-24' : 'w-20'}`}>
        <div className="flex items-center justify-between mb-4">
          <Link href="/dashboard" className="flex items-center">
            <Image
              ...
              width={0}
              height={0}
              className={`h-6 w-6 min-w-[24px] block ${isCollapsed ? 'ml-2 mr-2' : ''}`}
              style={{ flexShrink: 0 }}
            />
            {!isCollapsed && mode !== 'hidden' && (
              <span className="pl-2 font-semibold text-xl tracking-wide">ASVA HUB</span>
            )}
          </Link>
        ...
        <div className="mt-auto pt-4">
          ...
          {mode === 'open' && (
            <Link href="/trash" className="flex items-center gap-2 rounded-lg text-sm px-2 py-1.5 hover:bg-accent hover:text-accent-foreground transition"> 
        ...
        <SheetContent side="left" className="w-64 bg-gradient-to-b from-[#02427E] to-[#05081A] p-4 text-white">
          <div className="flex items-center justify-between mb-4">
            <Link href="/dashboard" className="flex font-bold items-center">
              <Image src="/asva logo.png" alt="ASVA Logo" width={24} height={24} className="h-6 w-6" />
